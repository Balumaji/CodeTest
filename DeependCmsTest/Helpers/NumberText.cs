 ...
 * Not completely mine
 * Credit : 
 * From https://github.com/robertgreiner/NumberText
 * 
 * 
 * */

namespace DeependCmsTest.Helpers {
    ...
    public class NumberText {
        ...
        public string ToText(decimal num)
        {
            try
            {
                int whole = (int)Math.Truncate(num);
                int dec = (int)((num - whole) * 100);

                StringBuilder bld = new StringBuilder();
                bld.Append(ToText(whole) + " dollars ");

                if (dec > 0)
                {
                    bld.Append("and " + ToText(dec) + " cents ");
                }
                return bld.ToString();
            }
            catch(Exception ex)
            ...
            return "Number may be invalid or too large to textify";
        }
